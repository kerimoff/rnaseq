# czbiohub/rnaseq: Reference Genomes Configuration

The czbiohub/rnaseq pipeline needs a reference genome for alignment and annotation. If not already available, start by downloading the relevant reference, for example from [illumina iGenomes](https://support.illumina.com/sequencing/sequencing_software/igenome.html).
...
```groovy
includeConfig '/path/to/czbiohub-rnaseq/conf/igenomes.config'
```
